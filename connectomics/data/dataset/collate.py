...
def collate_fn_train_tgt(batch):
    return TrainBatch_tgt(batch)
...
class TrainBatch_tgt:
    def __init__(self, batch):
        pos, out_input, out_input_aug = zip(*batch)
        self.pos = pos
        self.out_input = torch.from_numpy(np.stack(out_input, 0))
        self.out_input_aug = torch.from_numpy(np.stack(out_input_aug, 0))
        
    # custom memory pinning method on custom type
    def pin_memory(self):
        self.out_input = self.out_input.pin_memory()
        self.out_input_aug = self.out_input_aug.pin_memory()
        return self
