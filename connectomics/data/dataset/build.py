...
from .dataset_unlabel_volume import UNLABLE_VolumeDataset
...
def _get_input(cfg,
    ...
    if mode == 'val':
        ...
        if source:
            ...
            img_name = cfg.DATASET.UNLABLE_IMAGE_NAME
            label_name = None
    ...
    if cfg.DATASET.DO_CHUNK_TITLE == 1:  # build TileDataset
        ...
        if source:
            volume, label, valid_mask = _get_input(
                cfg, mode, rank, dir_name_init, img_name_init, min_size=sample_volume_size, source=source)
            # breakpoint()
            ...
            volume, label, valid_mask = _get_input(
                cfg, mode, rank, dir_name_init, img_name_init, min_size=sample_volume_size, source=source)
            dataset = UNLABLE_VolumeDataset(volume=volume, label=None, valid_mask=None,
                                    iter_num=iter_num, **shared_kwargs, **dataset_options)
...
def build_dataloader(cfg, augmentor=None, mode='train', dataset=None, rank=None,
                     dataset_class=VolumeDataset, dataset_options={}, cf=collate_fn_train, source=True):
    r"""Prepare dataloader for training and inference.
    """
    assert mode in ['train', 'val', 'test']
    print('Mode: ', mode)
    if not source:
        cf = collate_fn_train_tgt
    if mode == 'train':
    ...
    if dataset is None: # no pre-defined dataset instance
        ...
        dataset = get_dataset(cfg, augmentor, mode, rank, dataset_class, dataset_options, source=source)
