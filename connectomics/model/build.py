...
from .arch import UNet3D, UNet2D, FPN3D, DeepLabV3, UNetPlus3D, UNetPlus2D, UNETR, SwinUNETR, UNet_PNI_aniso, UNet_PNI_iso, UNet_PNI_iso_con, UNet_PNI_iso_noda, RSUNet
...
def build_model(cfg, device, rank=None):
    ...
    if model_arch == 'superhuman_iso':
        print('load isotropic superhuman!')
        model = UNet_PNI_iso(in_planes=1,
                            out_planes=1,
                            ...
                            if_sigmoid=True)
    elif model_arch == 'superhuman_iso_noda':
        print('load isotropic superhuman without DA!')
        model = UNet_PNI_iso_noda(in_planes=1,
                            out_planes=1,
                            filters=[28, 36, 48, 64, 80],
                            upsample_mode='bilinear',
                            decode_ratio=1,
                            ...
                            init_mode='kaiming_normal',
                            if_sigmoid=True)
    elif model_arch == 'superhuman_iso_con':
        print('load isotropic superhuman with consistency loss!')
        model = UNet_PNI_iso_con(in_planes=1,
                            out_planes=1,
                            filters=[28, 36, 48, 64, 80],
                            upsample_mode='bilinear',
                            decode_ratio=1,
                            ...
                            init_mode='kaiming_normal',
                            if_sigmoid=True)
    elif model_arch == 'superhuman_aiso' :
        print('load anisotropic superhuman!')
        model = UNet_PNI_aniso(in_planes=1,
                            out_planes=1,
                            filters=[28, 36, 48, 64, 80],
                            upsample_mode='bilinear',
                            decode_ratio=1,
