...
LastEditTime: 2023-04-06 20:44:31
'''
...
class UNet_PNI_iso(nn.Module):  # deployed PNI model
    ...
    def forward(self, x):
        if self.training:
            ...
            # first layer
            embed_src = self.embed_in(x_src)            # [28, 160, 160, 160]
            embed_tgt = self.embed_in(x_tgt)            # [28, 160, 160, 160]

            mu_src, sig_src = mean_std(embed_src)
            mu_tgt, sig_tgt = mean_std(embed_tgt)
            embed_src_da = (embed_src - mu_src) / sig_src
            embed_src_da = embed_src_da * sig_tgt + mu_tgt

            # second layer
            conv0_src = self.conv0(embed_src_da)        # [28, 160, 160, 160]
            conv0_tgt = self.conv0(embed_tgt)           # [28, 160, 160, 160]

            mu_src, sig_src = mean_std(conv0_src)
            mu_tgt, sig_tgt = mean_std(conv0_tgt)
            ...
            return out_src_pre, out_src_post
        else:
