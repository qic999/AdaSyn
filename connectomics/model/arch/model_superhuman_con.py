...
LastEditTime: 2023-04-06 23:09:04
'''
...
class UNet_PNI_iso(nn.Module):  # deployed PNI model
    ...
    def forward(self, x):
        if self.training:
            ...
            # first layer
            embed_src = self.embed_in(x_src)
            embed_tgt_aug = self.embed_in(x_tgt_aug)
            with torch.no_grad():
                embed_tgt_oin = self.embed_in(x_tgt_oin)

            mu_src, sig_src = mean_std(embed_src)
            mu_tgt, sig_tgt = mean_std(embed_tgt_oin)
            embed_src_da = (embed_src - mu_src) / sig_src
            embed_src_da = embed_src_da * sig_tgt + mu_tgt

            # second layer
            conv0_src = self.conv0(embed_src_da)
            conv0_tgt_aug = self.conv0(embed_tgt_aug)
            with torch.no_grad():
                conv0_tgt_oin = self.conv0(embed_tgt_oin)

            mu_src, sig_src = mean_std(conv0_src)
            mu_tgt, sig_tgt = mean_std(conv0_tgt_oin)
            conv0_src_da = (conv0_src - mu_src) / sig_src
            conv0_src_da = conv0_src_da * sig_tgt + mu_tgt

            out_src_pre, out_src_post = self.forward_main(conv0_src_da)
            out_tgt_aug_pre, out_tgt_aug_post = self.forward_main(conv0_tgt_aug)
            with torch.no_grad():
                out_tgt_oin_pre, out_tgt_oin_post = self.forward_main(conv0_tgt_oin)

            return out_src_pre, out_src_post, out_tgt_aug_pre, out_tgt_aug_post, out_tgt_oin_pre, out_tgt_oin_post
        else:
